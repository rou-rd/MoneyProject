...
getRemainingAmount(category?: Category): number {
  if (!category || category.budget === undefined) {
    return 0; // ou une valeur par défaut
  }
  return (category.budget - (category.spent || 0));
}
getAbsoluteRemainingAmount(category: Category): number {
  return Math.abs(this.getRemainingAmount(category));
}
}